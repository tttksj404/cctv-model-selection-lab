...
class ZoneSearchPlanRequest(ZoneSearchModel):
    ...
    search_to: datetime
    zone_count: int = Field(default=4, ge=4, le=4)
    cameras_per_zone: int = Field(default=4, ge=4, le=4)
    ...
    def validate_search_topology(self) -> Self:
        ...
        for zone_id in range(1, self.zone_count + 1):
            ...
            if positions != set(range(1, self.cameras_per_zone + 1)):
...
def _validate_zone_cameras(
    ...
    if positions != {1, 2, 3, 4}:
        message = "zone camera positions must be 1 through 4"
        raise ValueError(message)
...
class CandidateEventRequest(ZoneSearchModel):
    ...
    camera_id: str = Field(min_length=1, max_length=100)
    zone_id: int = Field(gt=0)
    camera_position: int = Field(ge=1, le=4)
    zone_cameras: tuple[ZoneCameraTarget, ...] = Field(min_length=4, max_length=4)
