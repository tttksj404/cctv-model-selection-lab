...
MODEL_CHECKPOINTS = {
    ...
    "clipreid-vit-b16-msmt17": "models/clip_reid/MSMT17_clipreid_ViT-B-16_60.pth",
}
FASTREID_CONFIGS = {
...
class ImageEncoder:
    def __init__(
        ...
        if self.family == "clip":
        ...
        elif self.family == "clipreid":
            root = clipreid_root or (
                Path(os.environ["CLIPREID_ROOT"])
                if os.environ.get("CLIPREID_ROOT")
                else None
            )
            if root is None:
                raise RuntimeError(
                    "CLIP-ReID requires --clipreid-root or CLIPREID_ROOT pointing "
                    "to a checked-out CLIP-ReID repository"
                )
            root = _resolve_model_root(root, "model")
            if str(root) not in sys.path:
                sys.path.insert(0, str(root))
            from config import cfg as clipreid_cfg
            from model.make_model_clipreid import make_model as make_clipreid_model
            from torchvision.transforms import Compose, Normalize, Resize, ToTensor

            config_path = root / "configs" / "person" / "vit_clipreid.yml"
            if not config_path.is_file():
                raise FileNotFoundError(f"CLIP-ReID config not found: {config_path}")
            cfg = clipreid_cfg.clone()
            cfg.merge_from_file(str(config_path))
            cfg.DATASETS.NAMES = "msmt17"
            cfg.TEST.NECK_FEAT = "before"
            cfg.freeze()
            weights = _require_local_checkpoint(checkpoint, "CLIP-ReID")
            ...
            self.model = make_clipreid_model(
                cfg,
                num_class=1041,
                camera_num=15,
                view_num=1,
    ...
    def encode(self, records: Sequence[Record], batch_size: int) -> np.ndarray:
        ...
        with torch.inference_mode():
            for start in range(0, len(records), batch_size):
                ...
                if self.family in {"reid", "solider", "clipreid"}:
                    ...
                    elif self.family == "clipreid" and self.tta == "hflip":
                        flipped = self.model(torch.flip(inputs, dims=(3,)))
                        features = torch.nn.functional.normalize(
                            features, dim=-1
