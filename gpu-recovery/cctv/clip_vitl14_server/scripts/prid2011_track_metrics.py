...
def calibrate_open_set(tracks: Sequence[TrackEmbedding]) -> Calibration:
    return calibrate_outcomes(_outcomes(tracks))


def calibrate_outcomes(outcomes: Sequence[QueryOutcome]) -> Calibration:
    if not outcomes:
        raise ValueError("calibration requires query outcomes")
...
def evaluate_retrieval(
    ...
    return evaluate_outcomes(_outcomes(tracks), calibration)
...
def evaluate_outcomes(
    ...
    if not outcomes:
        raise ValueError("evaluation requires query outcomes")
