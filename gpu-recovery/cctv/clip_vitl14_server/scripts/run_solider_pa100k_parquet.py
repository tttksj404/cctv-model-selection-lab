...
def _extract_split(
    ...
    with torch.inference_mode():
        for batch in parquet.ParquetFile(path).iter_batches(batch_size=batch_size, columns=columns):
            ...
            images = torch.stack(
                [_image_tensor(rows["image"][index]) for index in range(count)]
            ).to(device)
...
def main() -> None:
    ...
    parser.add_argument(
        "--vendor-root",
        type=Path,
        default=Path("experiments/vendor/SOLIDER-PersonAttributeRecognition"),
    )
    parser.add_argument(
    ...
    model = _load_backbone(args.vendor_root.resolve(), args.checkpoint.resolve()).to(device)
    model.eval()
