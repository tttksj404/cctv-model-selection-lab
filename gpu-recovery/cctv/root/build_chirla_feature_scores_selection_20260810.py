...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="minimum number of identities required by the selected evaluation gate",
    )
    return parser.parse_args()


def _strict_split(
    records: Sequence[Record], minimum_identities: int
) -> tuple[list[Record], list[Record], list[str]]:
    ...
    if len(identities) < minimum_identities:
        raise ValueError(
            f"evaluation gate requires at least {minimum_identities} identities, "
...
def main() -> None:
    ...
    gallery, queries, identities = _strict_split(records, args.minimum_identities)
    gallery_indices = {
        identity: [
