...
PolicyName: TypeAlias = Literal[
    "static_representative", "deployed_runtime", "pure_information_gain"
]
...
def _runtime_evidence() -> dict[str, str | list[str]]:
    try:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Selection plus sealed replay of static, deployed, and pure-EIG policies"
    )
    ...
    by_cell: defaultdict[
        tuple[CohortName, str, str, PolicyName], list[EpisodeOutcome]
    ] = defaultdict(list)
    for outcome in outcomes:
        by_policy[(outcome.cohort, outcome.policy)].append(outcome)
        by_cell[
            (outcome.cohort, outcome.scenario, outcome.operating_point, outcome.policy)
        ].append(outcome)
    selection_aggregate = {
        policy: _round_metrics(_aggregate(by_policy[("selection", policy)]))
        for policy in POLICIES
    }
    ...
    result = {
        ...
        "pairedEpisodeCount": len(cohorts)
        * len(scenarios)
        * len(points)
