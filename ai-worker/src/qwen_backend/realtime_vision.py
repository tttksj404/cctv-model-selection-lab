...
class RealtimeVisionConfig:
    ...
    minimum_box_area_fraction: float = 0.01

    def __post_init__(self) -> None:
        if not self.tracker.strip():
            raise ValueError("tracker configuration is required")
        if not 0.0 <= self.confidence <= 1.0:
            raise ValueError("confidence must be between 0 and 1")
        if self.image_size < 1:
            raise ValueError("image_size must be positive")
        if self.score_every_frames < 1:
            raise ValueError("score_every_frames must be positive")
        if self.track_ttl_frames < 1:
            raise ValueError("track_ttl_frames must be positive")
        if not 0.0 <= self.minimum_box_area_fraction <= 1.0:
            raise ValueError("minimum_box_area_fraction must be between 0 and 1")
...
class RealtimeVisionEngine:
    ...
    def process(self, frame_bgr: np.ndarray, frame_number: int) -> FrameInference:
        ...
        for coordinates_row, confidence, track_id in zip(
            ...
            if (
                crop.size == 0
                or box_area < height * width * self._config.minimum_box_area_fraction
            ):
                continue
