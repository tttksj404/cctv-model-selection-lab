...
QWEN_CANDIDATE_QWEN_REMOTE_API_KEY=<redacted>
```
...
```powershell
uv run python -c "from dotenv import load_dotenv; load_dotenv('C:/path/to/<worker-env-file>'); from qwen_backend.multi_model_candidate_engine import create_engine; e=create_engine(); b=e._base._get_clip_bundle(); e._base._get_detector(); print(e._get_fine(b) is not None)"
```
