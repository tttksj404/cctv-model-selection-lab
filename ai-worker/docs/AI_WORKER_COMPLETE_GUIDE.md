...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
  ...
  -OutputPath <local-user-path>\Desktop\EyesOnU-AI-Worker.exe
```
...
```powershell
cd <local-user-path>\Desktop\2학기\S15P11A204-deploy-ai-worker-env-fix\ai-worker
uv sync --extra realtime --frozen
ffmpeg -version
...
`<worker-env-file>` 같은 실제 비밀 파일은 문서에 복사하지 않는다. 다음 종류의 값이
필요하다.
