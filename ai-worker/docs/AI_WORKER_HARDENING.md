...
- 노트북 reference 파일 크기, 중앙 API URL, API key placeholder를 settings 단계에서 검증한다. 잘못된 `Content-Length`도 다운로드를 시작하지 않고 실패한다.
...
- Python worker와 Java backend의 worker path/header/schema version/max-size 값을 각 언어 내부의 단일 protocol constant로 모았다. schema version을 바꿀 때는 두 상수를 함께 변경하고 양쪽 contract test를 통과해야 한다.
...
- `/api/v1/ai-worker/jobs`, `X-AI-Worker-Key`, schema version, 후보·오류 메시지 최대 길이는 중앙 서버와 공유하는 **프로토콜 계약**이다. 환경변수로 바꾸면 worker와 backend가 서로 다른 API를 호출할 수 있으므로 상수로 유지한다.
...
1. `ai-worker/.env.example`을 기준으로 실제 `.env`를 만들고 모델 경로와 중앙 API 주소를 환경에 맞게 지정한다.
...
5. 실제 중앙 서버에 대한 claim preflight는 코드 테스트와 별개다. HTTP 403이면 노트북 코드에서 임의 우회하지 말고 서버 배포의 worker key·권한·header 계약을 수정해야 한다.
