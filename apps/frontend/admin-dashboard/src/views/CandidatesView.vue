  ...
  <section class="content-panel">
    ...
    <div v-if="filters.view === 'card'" class="candidate-grid">
      <button v-for="item in visible" :key="item.id" class="candidate-card" @click="router.push(`/admin/candidates/${item.id}`)">
        ...
        <p class="candidate-location">{{ item.location }}</p>
        <b :class="['similarity-score', similarityTone(item.similarity)]">{{ item.similarity }}%</b>
      </button>
    </div>
    ...
    <div v-else class="table-scroll">
      <table class="case-table">
        <thead>
          <tr><th>사건</th><th>CCTV</th><th>카메라 위치 주소</th><th>탐지 시각</th><th>유사도</th><th>상태</th><th></th></tr>
        </thead>
        <tbody>
          <tr v-for="item in visible" :key="item.id">
            ...
            <td :class="['similarity-score', similarityTone(item.similarity)]">{{ item.similarity }}%</td>
            <td>{{ item.review }}</td>
