...
const initialNotice = {
  "password-changed": "비밀번호가 변경되었습니다. 새 비밀번호로 다시 로그인해 주세요."
}[initialReason] || "";
const error = ref(initialError);
const notice = ref(initialNotice);
...
<template>
  <main class="login-page">
    <form class="login-card" autocomplete="off" @submit.prevent="submit">
      <div class="brand-mark"><img :src="LOGO_URL" alt="Eyes On U" /></div>
      <h1>관리자 로그인</h1>
      ...
      <div v-if="notice" class="form-success" role="status">{{ notice }}</div>
