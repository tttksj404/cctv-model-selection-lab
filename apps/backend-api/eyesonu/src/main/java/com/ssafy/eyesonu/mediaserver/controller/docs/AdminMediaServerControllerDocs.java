...
@Tag(name = "관리자 Media Server", description = "관리자용 Media Server 조회 API")
public interface AdminMediaServerControllerDocs {

	@Operation(
			summary = "활성 Media Server 옵션 조회",
			description = "카메라 등록 화면에서 선택할 ACTIVE Media Server를 코드 오름차순으로 조회합니다. Device Key 정보는 반환하지 않습니다.",
			security = @SecurityRequirement(name = SwaggerConfig.SESSION_SCHEME))
	@ApiResponses({
			@io.swagger.v3.oas.annotations.responses.ApiResponse(
					responseCode = "200", description = "활성 Media Server 옵션 조회 성공", useReturnTypeSchema = true),
			@io.swagger.v3.oas.annotations.responses.ApiResponse(
