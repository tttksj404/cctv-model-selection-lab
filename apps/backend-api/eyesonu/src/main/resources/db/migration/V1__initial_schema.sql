...
CREATE TABLE `cameras` (
    ...
    `raspberry_id` VARCHAR(100) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
    `device_key_hash` VARCHAR(255) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
    `latitude` DECIMAL(10, 7) NOT NULL,
    `longitude` DECIMAL(10, 7) NOT NULL,
    `address` VARCHAR(255) NOT NULL,
    `rtsp_url` VARCHAR(500) COLLATE utf8mb4_bin NOT NULL,
    ...
    `updated_at` DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6)
        ON UPDATE CURRENT_TIMESTAMP(6),
    PRIMARY KEY (`id`),
    CONSTRAINT `uk_cameras_raspberry_id` UNIQUE (`raspberry_id`),
