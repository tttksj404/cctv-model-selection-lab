...
- 임베디드 응답의 `prompt`, `exclusionPrompt`는 백엔드가 영어로 변환한다. `prompt`는 반드시 `성별 → 상의 색 → 상의 형태(short sleeve 또는 long sleeve) → 하의 색` 순서로 전달한다. 예: `남성, 검은색 반팔 상의와 검은색 청바지` → `a man wearing a black short sleeve top and black pants`.
