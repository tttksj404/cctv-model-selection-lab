...
Jetson은 각 `uploadUrl`에 응답의 `contentType`으로 이미지 바이너리만 `PUT`한다. 모든 업로드가 성공한 뒤 발급받은 `objectKey`를 사용해 후보 이벤트를 등록한다. URL이 만료되면 같은 내용으로 이 API를 다시 호출한 뒤 업로드를 재시도한다.
