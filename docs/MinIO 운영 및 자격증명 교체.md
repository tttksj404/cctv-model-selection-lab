...
이 문서는 백엔드가 사용하는 MinIO 운영값을 변경하거나 장애를 복구할 때 따른다. 미디어 서버는 MinIO 장기 자격증명을 보유하지 않고 백엔드가 발급한 presigned PUT URL만 사용한다. MinIO는 S3 API를 제공하지만 이 프로젝트의 운영 대상은 MinIO 하나로 고정한다.
...
| 공개 URL | `MINIO_PUBLIC_ENDPOINT` | 백엔드가 발급하고 외부 클라이언트가 사용 | 브라우저와 미디어 서버가 접근할 수 있는 HTTPS 주소를 사용한다. 비워 두지 않는다. |
| API 포트 | `MINIO_API_PORT` | Compose | MinIO API 포트와 보안 그룹 설정을 일치시킨다. |
| 버킷 | `MINIO_BUCKET` | 백엔드 | 환경별 버킷을 분리하고 버킷명을 배포 로그에 출력하지 않는다. |
...
| 애플리케이션 키 | `MINIO_APP_ACCESS_KEY`, `MINIO_APP_SECRET_KEY` | 백엔드 | root 계정 대신 최소 권한 애플리케이션 계정을 사용하며 미디어 서버에는 배포하지 않는다. |
...
4. 새 설정으로 배포하고 MinIO health, 기존 객체 `statObject`, presigned URL 다운로드, 미디어 서버의 URL 발급 → 공용 HTTPS PUT → 완료 등록을 순서대로 확인한다.
5. 기존 키를 사용하는 signer 인스턴스를 모두 중지한 뒤 최소 900초를 기다리고, 진행 중인 업로드도 모두 끝난 것을 확인한 다음 MinIO에서 기존 키를 폐기한다. presigned URL 만료 설정을 900초보다 길게 사용한다면 그 만료시간만큼 기다린다.
...
6. 완료 등록까지의 쓰기 검증은 격리된 dev/test 환경의 전용 테스트 카메라와 UUID로 수행한다. 검증 후에는 생성된 녹화·멱등 DB 레코드와 테스트 객체를 함께 정리해 dangling 레코드를 남기지 않는다. 운영 복구에서는 기존 녹화의 조회·재생 URL과 `statObject`만 검증하며 테스트 완료 레코드를 만들지 않는다.
